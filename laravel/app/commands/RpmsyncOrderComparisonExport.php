...
class RpmsyncOrderComparisonExport extends Command {
                            /*
                              From Magento:
     ...
     * @var string
     */
    protected $name = 'rpmsync:order-comparison-export';
     ...
     * @var string
     */
    protected $description = 'Compare completed orders in Magento vs receipts in Retail Pro and export the results as CSV';
        ...
        try {
            ...
            if (count($allMageOrders) > 0) {
                ...
                foreach ($allMageOrders as $mageOrder) {
                    ...
                    $metaArray['MAGETOTAL'] = (float) $mageOrder->total;
                    $metaArray['RPTOTAL'] = null;
    ...
    protected function report($i, $c, $results, $metaArray) {
        ...
        if (count($results['fail']) > 0) {
            ...
            */
            ...
            $failString = $results['fail'][0]['datapoint'];
	   ...
	   //$this->info("PASS $i on $c $metaString");
        ...
        if ($metaArray['RPTOTAL']) {
            $metaArray['RPTOTAL'] = number_format($metaArray['RPTOTAL'], 2, '.', '');
        }
        ...
        $this->info(
            ...
            $metaArray['SWC'] . ', ' . 
            $metaArray['SWT'] . ', ' . 
            number_format($metaArray['MAGETOTAL'] , 2, '.', '') . ', ' . 
            $metaArray['RPTOTAL'] . ', ' . 
            $passFail . ', ' .
            $failString
        );
