...
$(document).ready(function(){
    ...
    //loadReport(storeNumber, asRangeType, lastSunday, nextSaturday, empCode);

    // Run the report
    $("#allstar-run").on('click', function(){
      var fromVal   = $("#date-from").val();
      var toVal     = $("#date-to").val();
      var empVal    = $("#emp").val().toUpperCase();
      var fromDate  = moment(fromVal, "MM-DD-YYYY");
      var toDate    = moment(toVal, "MM-DD-YYYY");

      loadReport(storeNumber, asRangeType, fromDate, toDate, empVal);
    });

    // Run the report on form submit
    $("#allstar-month").submit(function(event){
...
function loadReport(storeNumber, asRangeType, asDateFrom, asDateTo, empCode){
    $("#allstar-run").hide();
    ...
    if (empCode.length < 5) {
        $("#report-header").html("Invalid Employee Code");
        $("#report-data").html("<tr><td><em>Unable to run report for Employee "+empCode+".</em></td></tr>");
        $("#allstar-run").show();
        return;
    }
    ...
    $("#report-header").html("All Star | Employee "+empCode+" (<span id='emp_name'></span>)<br>From " + fromTitle +" to "+toTitle);
    ...
    var url  = "/lsvc/reports-all-star-by-month/"+storeNumber+"/"+asRangeType+"/"+fromEncoded+"/"+toEncoded+"/"+empCode;
...
function showReport(data){
  ...
  html.push(
      ...
      "<tr>",
          ...
          "<th class='text-right'><strong>Month</strong></th>",
  ...
  for (var row=0; row<data.details.length; row++)
  {
      ...
      $("#emp_name").html(tr.RPRO_FULL_NAME);
      html.push(
          "<tr>",
              "<td nowrap class='text-right'></td>",

              "<td nowrap class='text-right'>"+tr.Year+"</td>",

              "<td nowrap class='text-right'>"+tr.Month+"</td>",
