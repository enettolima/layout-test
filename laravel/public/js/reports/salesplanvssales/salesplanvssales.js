function loadReport(storeNumber)
{
    ...
    reportRequest.done(function(response){
        ...
        for (var row=0; row<response.data.length; row++) 
        {
            ...
            summaryHTML.push(
                "<tr>",
                    /* Month */        "<td class='text-right'>"+tr.Month+"/"+tr.Year+"</td>",
                    /* Sales */        "<td class='text-right'>"+tr.pSALES.parsed+"</td>",
                    /* Budget */       "<td class='text-right'>"+tr.pBUDGET.parsed+"</td>",
                    /* Difference */   "<td class='text-right "+ ((tr.pSTR_AMT_Diff.isNegative) ? "text-danger" : "text-success") +"'>"+tr.pSTR_AMT_Diff.parsed+"</td>",
                    /* Difference % */  "<td class='text-right "+ ((tr.pSTR_PER_TO_BUDGET.isNegative) ? "text-danger" : "text-success") +"'>"+tr.pSTR_PER_TO_BUDGET.parsedDec+"</td>",
                    /* LY Sales MTD */ "<td class='text-right'>"+tr.pLY_SALES_MTD.parsed+"</td>",
                    /* Total Budget */ "<td class='text-right'>"+tr.pTOTAL_BUDGET.parsed+"</td>",
                "</tr>"
            );
